...
export default function SlideCanvas({ elements, onChange, selectedId, onSelect }: SlideCanvasProps) {
  ...
  const handleResizeMouseDown = (e: React.MouseEvent, element: SlideElement, _corner: string) => {
    e.stopPropagation();
    onSelect(element.id);
    setIsResizing(true);
          ...
          {element.type === "shape" && (
            <div
              ...
              style={{
                ...
                borderRadius: element.style.shapeType === "circle" ? "50%" :
                              element.style.shapeType === "rounded" ? "12px" : "0",
              }}
            />
          )}
...
interface SlideToolbarProps {
  onAddElement: (type: "text" | "shape", shapeType?: "rectangle" | "circle" | "rounded") => void;
  onDelete: () => void;
  hasSelection: boolean;
}

export function SlideToolbar({ onAddElement, onDelete, hasSelection }: SlideToolbarProps) {
  return (
