...
import { Play, X, Plus, Trash2, Copy, FileDown, Loader2 } from "lucide-react";
...
export default function PythonRunner({ isOpen, onToggle, onInsertOutput }: PythonRunnerProps) {
  const [pyodide, setPyodide] = useState<PyodideInterface | null>(null);
  const [loading, setLoading] = useState(false);
  const [cells, setCells] = useState<CodeCell[]>([
    { id: crypto.randomUUID(), code: "# Write Python code here\nprint('Hello, World!')", output: "", error: false, running: false }
  ]);
...
capture = CaptureOutput()
  ...
  const runCode = useCallback(async (cellId: string) => {
    if (!pyodide) return;
    ...
    // Mark as running
    setCells(prev => prev.map(c => c.id === cellId ? { ...c, running: true, output: "", error: false } : c));
...
output
      ...
      setCells(prev => prev.map(c =>
        c.id === cellId ? { ...c, output: String(result || ""), running: false, error: false } : c
      ));
    } catch (err) {
      setCells(prev => prev.map(c =>
        c.id === cellId ? { ...c, output: String(err), running: false, error: true } : c
      ));
    }
  }, [pyodide, cells]);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {cells.map((cell, index) => (
            <div key={cell.id} className="border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden">
              {/* Cell header */}
              ...
              {/* Output */}
              {cell.output && (
                <div className={`border-t border-zinc-200 dark:border-zinc-700 ${cell.error ? "bg-red-50 dark:bg-red-900/20" : "bg-zinc-50 dark:bg-zinc-800"}`}>
                  <div className="flex items-center justify-between px-3 py-1 border-b border-zinc-200 dark:border-zinc-700">
                    <span className="text-xs text-zinc-500">Out [{index + 1}]</span>
                    <div className="flex items-center gap-1">
                      <button
                  ...
                  <pre className={`px-3 py-2 text-sm font-mono overflow-x-auto ${cell.error ? "text-red-600 dark:text-red-400" : "text-zinc-800 dark:text-zinc-200"}`}>
                    {cell.output}
                  </pre>
                </div>
              )}
