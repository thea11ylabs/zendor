...
function parseContentWithMath(content: string): Array<{ type: "text" | "math-inline" | "math-display"; value: string }> {
  const parts: Array<{ type: "text" | "math-inline" | "math-display"; value: string }> = [];
...
export default function DocEditor({ content, onChange }: DocEditorProps) {
  ...
  const handleMathEdit = useCallback((blockIndex: number, oldMath: string, newMath: string, isDisplay: boolean) => {
    ...
    if (parts.length === 1 && parts[0].type === "math-display") {
      return (
        <MathBlock
          ...
          value={parts[0].value}
          display={true}
          onEdit={(newValue) => handleMathEdit(blockIndex, parts[0].value, newValue, true)}
        />
      );
    }
          ...
          if (part.type === "text") {
            ...
            return (
              <MathBlock
                ...
                value={part.value}
                display={false}
                onEdit={(newValue) => handleMathEdit(blockIndex, part.value, newValue, false)}
              />
            );
          } else {
            return (
              <MathBlock
                ...
                value={part.value}
                display={true}
                onEdit={(newValue) => handleMathEdit(blockIndex, part.value, newValue, true)}
              />
            );
          }
        })}
